...
int main(void) {
  ...
  FT_Face font;
  if (FT_New_Face(ft, "../assets/Alagard.ttf", 0, &font)) {
  ...
  // Init Text Buffers
  uint32_t textShaderID = LoadShaders("./textVertexShader.glsl", "./textFragmentShader.glsl");
  uint32_t textVAO, textVBO;
  glGenVertexArrays(1, &textVAO);
  glGenBuffers(1, &textVBO);
  glBindVertexArray(textVAO);
  glBindBuffer(GL_ARRAY_BUFFER, textVBO);
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 6 * 4, nullptr, GL_DYNAMIC_DRAW); // One Quad = 6 vertices = 6 * 4 floats
  ...
  lights.addSpotLight(glm::vec3 { 0, 5, 0}, glm::vec3 { 1, 1, 0 }, glm::vec3 { 0.8f, 0.8f, 0.8f }, 1.0f, 0.1f, 0.02f, Maths::radians(45));
  ...
  BoxCollider2D playerCollider = BoxCollider2D(currentCamera().position, glm::vec2(0.25f));
  ...
  box.addTexture("../assets/stones_diffuse.png", "diffuse");
  ...
  colliders.push_back(BoxCollider2D({ 0, 0, 0 }, { 1.0f, 1.0f }));
  ...
  textQueue.push_back(TextRenderData { std::string("Not \na test"), glm::ivec2(10, 50), 1.0, glm::vec3(1.0f, 0.5f, 0.5f) });
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (Maths::sqrMagnitude(movementInput) >= 0.01f) {
      canMove = true;
      glm::vec3 moveDir3 = currentCamera().forward * movementInput.y + currentCamera().right * movementInput.x;
      glm::vec2 moveDir = Maths::normalize(glm::vec2(moveDir3.x, moveDir3.z));
      for (const BoxCollider2D& collider : colliders) {
        glm::vec2 obstacleDir = Maths::normalize(glm::vec2(collider.position.x, collider.position.z) - glm::vec2(currentCamera().position.x, currentCamera().position.z));
    ...
    if (canMove && camera == FPS) {
      glm::vec3 moveDir = currentCamera().forward * movementInput.y + currentCamera().right * movementInput.x;
      moveDir.y = 0;
      currentCamera().position += walkSpeed * deltaTime * Maths::normalize(moveDir);
    ...
    objects.front().tint = Maths::hslToRGB(glm::vec3(hue, 1, 0.5f));
    objects.front().rotation = Quaternion(0, time * M_PI);
    objects.front().position = glm::vec3(0, sinf(time * M_PI) * 0.25f + 0.75f, 0);
    ...
    glUniformMatrix4fv(glGetUniformLocation(textShaderID, "projection"), 1, GL_FALSE, glm::value_ptr(textProjection));
    for (TextRenderData& data : textQueue) {
      float x = data.position.x;
      float y = data.position.y;
      glUniform3fv(glGetUniformLocation(textShaderID, "textColour"), 1, glm::value_ptr(data.colour));
      glActiveTexture(GL_TEXTURE0);
      glBindVertexArray(textVAO);
      for (std::string::iterator it = data.text.begin(); it != data.text.end(); it++) {
        ...
        glm::vec2 pos = glm::vec2(x + ch->bearing.x * data.scale, y - (ch->size.y -ch->bearing.y) * data.scale);
